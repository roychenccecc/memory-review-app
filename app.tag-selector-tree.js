...
function normalizeCodexReviewDraft(raw) {
  ...
  return {
    ...
    date: String(raw.date || toDateInput(new Date())).slice(0, 10),
    subject,
    chapter,
    title,
    target: {
...
function showCodexReviewDraft(draft) {
  ...
  summary.innerHTML = `
    ...
    <div class="meta">
      <span>${escapeHtml(draft.date || toDateInput(new Date()))}</span>
...
async function confirmActiveCodexReviewDraft() {
  ...
  form.append(
    ...
    bridgeCompletionInput("date", draft.date || toDateInput(new Date())),
